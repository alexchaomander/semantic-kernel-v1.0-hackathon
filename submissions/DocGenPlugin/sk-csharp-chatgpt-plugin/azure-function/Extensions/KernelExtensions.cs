...
public static class KernelExtensions
{
        ...
        this Kernel kernel, string pluginName, string promptDirectory)
    {
        ...
        foreach (string dir in directories)
        {
            ...
            //var template = new BasicPromptTemplate(File.ReadAllText(promptPath), config, kernel.PromptTemplateEngine);

            // Prepare lambda wrapping AI logic
            //var functionConfig = new SemanticFunctionConfig(config, template);
            
            var fun = kernel.CreateFunctionFromPrompt(File.ReadAllText(promptPath), config.ExecutionSettings.FirstOrDefault());
